"""Drive the FastAPI surface end to end against the stub backend.

FACE_ENGINE=stub supplies vectors instead of running a model, so identity
scenarios can be scripted exactly; the routing, request schemas and response
bodies exercised below are the real ones.  Each test image carries a tag in its
first pixel and the stub maps that tag to a chosen vector.

Most of this file tests the identity path; the liveness gate has its own
section at the end, driven separately so it needs no weights or onnxruntime.
"""
import base64
import io
import os
...
# --- what the stub hands back ------------------------------------------------
VECTORS = {}          # image tag -> embedding
DIM = engine.EMBEDDING_DIM

# One face by default; the two-face list puts a bystander in frame.
ONE_FACE = [(10, 210, 210, 10)]
TWO_FACES = ONE_FACE + [(10, 380, 120, 240)]
STATE["boxes"] = list(ONE_FACE)
# The tag is painted into the first pixel so the stub can tell images apart;
# bystanders reuse the subject's vector, which is enough for the plumbing.
STATE["vector"] = lambda array, index: VECTORS[int(array[0, 0, 0])]

# --- helpers -----------------------------------------------------------------
DIRECTION = np.random.default_rng(7).normal(size=DIM).astype(np.float32)
DIRECTION /= np.linalg.norm(DIRECTION)
BASE = np.random.default_rng(1).normal(size=DIM).astype(np.float32)
...
CITRA_PROFILE = make_image(31, np.random.default_rng(99).normal(size=DIM).astype(np.float32) * 0.4)
...
check("legacy tolerance comes from the engine's own scale",
      body["data"]["tolerance"] == config.LEGACY_TOLERANCE,
      f'{body["data"]["tolerance"]} vs {config.LEGACY_TOLERANCE}')
