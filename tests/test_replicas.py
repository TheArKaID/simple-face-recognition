...
def _stub_face_recognition():
    """engine.py imports face_recognition at module scope; dlib is not needed
    for these tests since vectors are supplied directly."""
    fake = types.ModuleType("face_recognition")
    fake.face_locations = lambda *a, **k: [(10, 210, 210, 10)]
...
def run_worker(db_path):
    ...
    def vec(seed, offset=0.0):
        ...
        v /= np.linalg.norm(v)
        if offset:
            d = np.random.default_rng(7).normal(size=128).astype(np.float32)
            d /= np.linalg.norm(d)
            v = v + d * offset
