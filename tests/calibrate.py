...
Run it inside the container, where the weights and onnxruntime live:
...
# new-images is the calibration set: 30 identities, 10 attendance selfies each,
# folder-per-person so identity is stated rather than guessed.  The older flat
# tests/images fixture stays for the liveness spoof samples and as a regression
# fixture, but it is not representative - it contains deliberately extreme
# expressions nobody produces at a clock-in, and calibrating to those put the
# thresholds 0.09 too high.
IMAGE_DIR = os.getenv(
    "FACE_CALIB_IMAGES",
    os.path.join(os.path.dirname(os.path.abspath(__file__)), "new-images"),
)
...
# Two layouts, and which is in use decides how identity is established:
#
#   flat    IMAGE_DIR/<identity><n>.jpg - identity guessed from the filename,
#           which is why ALIASES exists, and why one mislabelled pair once
#           inverted a whole conclusion here
#   nested  IMAGE_DIR/<identity>/*.jpg  - identity is the folder: stated, not
#           inferred, so there is nothing to get wrong
NESTED = bool(glob.glob(os.path.join(IMAGE_DIR, '*', '*.jpg')))
...
_candidates = (sorted(glob.glob(os.path.join(IMAGE_DIR, '*', '*.jpg'))) if NESTED
...
def stem_of(path):
    """Unique key for a photo; carries the folder in the nested layout."""
    base = os.path.splitext(os.path.basename(path))[0]
    return f"{os.path.basename(os.path.dirname(path))}/{base}" if NESTED else base
...
print(f"layout: {'nested (folder = identity)' if NESTED else 'flat (name = identity)'}  dir: {IMAGE_DIR}")
